...
<?php 
	...
	$field_duration = (int)  get_option('field_duration_1', 1000);

	$field_show_caption = get_option('field_show_caption_1',1);

    $field_show_bullet = get_option('field_show_bullet_1',1);

	$field_change_bullet = get_option('field_change_bullet_1',1);

    $field_show_arrows = get_option('field_show_arrows_1', 1);

	$field_change_arrows = get_option('field_change_arrows_1',1);


	$field_transition_effect = get_option('field_transition_effect_1', 'ParabolaStairsIn');

	$field_category = get_option('field_category_1');

	$field_img_size = get_option('field_img_size_1');

	$field_link_caption = get_option('field_link_caption_1',1);
	
?>

<!-- <script src="<?php //echo plugin_dir_url( __FILE__ ).'../js/jssor.slider-21.1.5.min.js'; ?>"> </script> -->
...
<script>
    ...
    /*console.log("Durée : "+field_duration);
    ...
    jssor_1_slider_init = function() {    	
        ...
        var jssor_1_Transition ;
        var jssor_1_options = {};

        /* --------- Defining jssor transitions effects ---------------*/

        if( field_transition_effect == "Fade"){
        	jssor_1_Transition = [{$Duration:field_duration, $Opacity:2}];
        }
        else if( field_transition_effect == "CollapseStairs"){
        	jssor_1_Transition = [{$Duration:field_duration,$Delay:12,$Cols:10,$Rows:5,$Clip:15,$SlideOut:true,$Formation:$JssorSlideshowFormations$.$FormationStraightStairs,$Easing:$Jease$.$OutQuad,$Opacity:2,$Assembly:2049},];
        }
        else if( field_transition_effect == "SwingInStairs"){
        	jssor_1_Transition = [{$Duration:field_duration,x:0.2,y:-0.1,$Delay:16,$Cols:10,$Rows:5,$Clip:15,$During:{$Left:[0.3,0.7],$Top:[0.3,0.7]},$Formation:$JssorSlideshowFormations$.$FormationStraightStairs,$Easing:{$Left:$Jease$.$InWave,$Top:$Jease$.$InWave,$Clip:$Jease$.$OutQuad},$Opacity:2,$Assembly:260,$Round:{$Left:1.3,$Top:2.5}},];
        }
        else if( field_transition_effect == "ParabolaStairsIn"){
        	jssor_1_Transition = [{$Duration:field_duration,x:-1,y:1,$Delay:50,$Cols:10,$Rows:5,$SlideOut:true,$Formation:$JssorSlideshowFormations$.$FormationStraightStairs,$Easing:{$Left:$Jease$.$InQuart,$Top:$Jease$.$InQuart,$Opacity:$Jease$.$Linear},$Opacity:2} ,];
        }
        else if( field_transition_effect == "Slide"){
        	jssor_1_Transition = [{$Duration:field_duration,x:-1,$Easing:$Jease$.$InQuad,$Opacity:2} , ];
        }
        else if( field_transition_effect == "DodgeDanceIn"){
        	jssor_1_Transition = [{$Duration:field_duration,x:0.3,y:-0.3,$Delay:20,$Cols:10,$Rows:5,$Clip:15,$During:{$Left:[0.2,0.8],$Top:[0.2,0.8]},$Formation:$JssorSlideshowFormations$.$FormationStraightStairs,$Easing:{$Left:$Jease$.$InJump,$Top:$Jease$.$InJump,$Clip:$Jease$.$OutQuad},$Opacity:2,$Assembly:260,$Round:{$Left:0.8,$Top:2.5}} , ];
        }
        else if( field_transition_effect == "ZoomIn"){
            jssor_1_Transition = [{$Duration:field_duration,$Zoom:11,$Easing:{$Zoom:$Jease$.$InCubic,$Opacity:$Jease$.$OutQuad},$Opacity:2} , ];
        }
        ...
        jssor_1_options = {
	        ...
	        $SlideshowOptions: {
		        ...
		        $Transitions: jssor_1_Transition,
		        $TransitionsOrder: 1
	        }
	   	};
        ...
        if( field_show_caption == 1 ){
        	...
        	jssor_1_options['$ThumbnailNavigatorOptions'] = captionOpt;
        ...
        if( field_show_bullet == 1 ){
        	...
        	jssor_1_options['$BulletNavigatorOptions'] = bulletOpt;
        ...
        if( field_show_arrows == 1 ){
        	...
        	jssor_1_options['$ArrowNavigatorOptions'] = arrowsOpt;
